...
try:
    ...
    with open("/home/gr106/Desktop/Scripts/70.txt",
              "w") as outfile:  # здесь каждый раз меняем название файла в засисимости от расстояния - "расстояние.txt"
        outfile.write('- Jet Lab\n\n')
        outfile.write(
            '- Experiment date = {}\n'.format(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))))
        outfile.write('- Number of samples in measure = {}\n'.format(
            samplesInMeasure))  # с этим нужно  разобраться, что это такое
